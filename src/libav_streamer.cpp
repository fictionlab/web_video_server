...
:ImageTransportImageStreamer(request, connection, node), format_context_(0), codec_(0),
  codec_context_(0), video_stream_(
...
void LibavStreamer::initialize(const cv::Mat & img)
{
  ...
  if (!format_context_) {
    async_web_server_cpp::HttpReply::stock_reply(
        async_web_server_cpp::HttpReply::internal_server_error)(request_,
                                                                                                         connection_,
                                                                                                         NULL,
        NULL);
  ...
  .header(
      "Pragma", "no-cache").header("Expires", "0").header("Max-Age", "0").header("Trailer",
      "Expires").header(
...
void LibavStreamer::sendImage(const cv::Mat & img, const rclcpp::Time & time)
{
  ...
  if (ret == AVERROR_EOF) {
    RCLCPP_DEBUG(node_->get_logger(), "avcodec_send_frame() encoder flushed");
  } else if (ret == AVERROR(EAGAIN)) {
    RCLCPP_DEBUG(node_->get_logger(), "avcodec_send_frame() need output read out");
  }
  if (ret < 0) {
  ...
  if (ret == AVERROR_EOF) {
    RCLCPP_DEBUG(node_->get_logger(), "avcodec_recieve_packet() encoder flushed");
  } else if (ret == AVERROR(EAGAIN)) {
    RCLCPP_DEBUG(node_->get_logger(), "avcodec_recieve_packet() need more input");
    got_packet = 0;
  }
